...
//! Calcu service. Specialized wrapper over substrate service.

use std::sync::Arc;
use std::time::Duration;
use sc_client_api::{RemoteBackend, ExecutorProvider};
use calcu_runtime::{self, opaque::Block, RuntimeApi};
...
type FullClient = sc_service::TFullClient<Block, RuntimeApi, CalcuExecutor>;
...
pub fn new_partial(config: &Configuration) -> Result<
    sc_service::PartialComponents<
        ...
        (
            impl Fn(
                calcu_rpc::DenyUnsafe,
                calcu_rpc::SubscriptionTaskExecutor
            ) -> calcu_rpc::RpcExtension,
            (
    ...
    let (client, backend, keystore_container, task_manager) =
        sc_service::new_full_parts::<Block, RuntimeApi, CalcuExecutor>(&config)?;
    ...
    let rpc_extensions_builder = {
        ...
        move |deny_unsafe, subscription_executor| -> calcu_rpc::RpcExtension {
            let deps = calcu_rpc::FullDeps {
                client: client.clone(),
                pool: transaction_pool.clone(),
                select_chain: select_chain.clone(),
                deny_unsafe,
                babe: calcu_rpc::BabeDeps {
                    babe_config: babe_config.clone(),
                    shared_epoch_changes: shared_epoch_changes.clone(),
                    keystore: keystore.clone(),
                },
                grandpa: calcu_rpc::GrandpaDeps {
...
pub fn new_light(mut config: Configuration) -> Result<TaskManager, ServiceError> {
    ...
    let (client, backend, keystore, mut task_manager, on_demand) =
        sc_service::new_light_parts::<Block, RuntimeApi, CalcuExecutor>(&config)?;
    ...
    let light_deps = calcu_rpc::LightDeps {
    ...
    let rpc_extensions = calcu_rpc::create_light(light_deps);
