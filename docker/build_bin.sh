...
function build_bin {
  ...
  docker pull calcuio/calcu-env:${TOOLCHAIN_VER}
  if [ $? -ne 0 ]; then
  ...
  RUN_OPTS="-v $BUILD_DIR:/opt/calcu -v $CACHEDIR:/opt/cache"
  ...
  docker run --workdir /opt/calcu --cidfile $CIDFILE -i -t --env CARGO_HOME=/opt/cache $RUN_OPTS calcuio/calcu-env:${TOOLCHAIN_VER} /bin/bash -c "$CMD"
  CID=`cat $CIDFILE`
  ...
  log_info "Calcu built at: $DIST_FILE"
}
